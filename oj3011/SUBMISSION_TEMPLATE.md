...
```text
Step 1: Input first color and second color
Step 2: If both colors can't mix it will show "Error"
...
```text
Simple testcase that doesn't have in example testcases
```
...
```text
Swap first color and second color, to check if it still works
```
...
```text
Both colors are the same, to check that it will show default color not "Error"
```
